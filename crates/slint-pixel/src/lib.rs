...
//! - `library_paths()`：供下游 crate 的 build.rs 注册 `@slint_pixel` 库导入。
...
//!
//! let ui = slint_pixel::PixelPainterWindow::new().unwrap();
//! slint_pixel::install_painter(&ui);
//! slint_pixel::install_title_bar_controls(&ui);
//! ui.run().unwrap();
//! ```
...
/// 返回 `@slint_pixel` 库路径表，供下游 crate 的 build.rs 注册。
///
...
///
/// 不需要手写实现——对由 `@slint_pixel` 生成的组件调用 [`impl_painter_ui!`]
...
/// 窗口缩放接线契约：带 `begin-resize(direction)` 回调的组件（配合 PixelWindowResize 热区）。
pub trait ResizeUi: slint::ComponentHandle {
    fn on_begin_resize(&self, cb: impl FnMut(slint::SharedString) + 'static);
}
...
/// ```
#[macro_export]
macro_rules! impl_resize_ui {
    ($ty:ty) => {
        impl $crate::ResizeUi for $ty {
            fn on_begin_resize(&self, cb: impl FnMut(::slint::SharedString) + 'static) {
                self.on_begin_resize(cb);
            }
        }
    };
}

/// 安装无边框窗口缩放：把 `PixelWindowResize` 热区的方向回调接到
/// winit 的 `drag_resize_window`（系统级缩放循环，跨平台）。
pub fn install_window_resize<T: ResizeUi + 'static>(ui: &T) {
    use slint::winit_030::winit::window::ResizeDirection;
    use slint::winit_030::WinitWindowAccessor;

    let weak = ui.as_weak();
    ui.on_begin_resize(move |dir| {
        let Some(ui) = weak.upgrade() else { return };
